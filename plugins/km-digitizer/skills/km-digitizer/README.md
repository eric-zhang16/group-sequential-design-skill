...
│   ├── verify_reconstruction.R # PASS/FAIL quality gate — MAE and MaxDev vs digitized curves
...
PNG image
  ...
  → verify_reconstruction.R → verification.json        (PASS/FAIL gate)
...
| `verify_reconstruction.R` | R | Compares digitized vs reconstructed KM curves; PASS if MAE < 3 pp and MaxDev < 5 pp |
...
### 3.5. Verify reconstruction (mandatory gate)
```bash
Rscript scripts/verify_reconstruction.R \
  output/<study>/km_digitized.json \
  output/<study>/ipd_combined.csv \
  output/<study>/
```
Must output `Overall: PASS` (MAE < 3 pp, MaxDev < 5 pp per arm) before proceeding.
...
**Python:**
```bash
pip install opencv-python numpy python-docx pandas matplotlib
```

**R:**
```r
install.packages(c(
  "IPDfromKM",   # IPD reconstruction from digitized KM curves
  "jsonlite",    # JSON I/O
  "survival",    # survfit, Surv
  "survminer",   # ggsurvplot KM plots
  "bshazard",    # B-spline smoothed hazard rate
  "ggplot2",     # plotting
  "gridExtra",   # side-by-side plot layout
